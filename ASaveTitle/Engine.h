...
public:
	~AClient_URL();
	AClient_URL(wchar_t *user_input);

	void Handle_Saver_URL(wchar_t *user_input);
...
/*
	- Добавил выбор между программами
	- Обнаружил кучу багов которые не пойми как появляються
		- Будем монитроить и фиксить

	- Добавить возможность выбора с какой программы стартувать
*/
// TASKS --- 12.06.2024 ---  --- curr ---
/*

	- Создать 30 словарей в которых будут слова от А до Z
		- Загружать все или те что нужны?)
...
/*
	- Stoped at http://loveread.me/read_book.php?id=92157&p=14
		...
		//while ( (next_line_begining = wcsstr(next_line_begining, L"<p class=MsoNormal>") ) != 0)
	//{
	//	const wchar_t* next_line_ending = wcsstr(next_line_begining, L"</");

	//	if (next_line_ending != 0)
	//	{
	//		length = (int)(next_line_ending - next_line_begining) - 19;
	//		wchar_t *extracted_line = new wchar_t[length + 1];
	//		wcsncpy_s(extracted_line, length + 1, (next_line_begining + 19), length);
	//		extracted_line[length] = L'\0';

	//		Extracted_Lines.push_back(extracted_line);

	//		delete[] extracted_line;
	//		next_line_begining = next_line_ending;
	//		if (next_line_ending == 0 || next_line_ending == L"</p>										</table>")
	//			return total_size;
	//	}
	//}
...
/*
	...
	AClient_URL client_url(temp);  // reset from url to User_Input
...
/*
--- MOST WANTED ---
X	- Зайти на основной сайт, и начать проверку по обновленным сериям
		- CURL
		- Закрасить их в списке другим цветом
X		- CURL зайти посмотреть последние добавление, если изменилось(добавили что то новое) дать пользувателю знать
